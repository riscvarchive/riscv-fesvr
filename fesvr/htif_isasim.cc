...
void htif_isasim_t::read_packet(packet_t* p, int expected_seqno)
{
...
void htif_isasim_t::write_packet(const packet_t* p)
{
  int size = p->data_size + offsetof(packet_t, data);
...
reg_t htif_isasim_t::read_cr(int coreid, int regnum)
{
  reg_t val;
...
void htif_isasim_t::write_cr(int coreid, int regnum, reg_t val)
{
