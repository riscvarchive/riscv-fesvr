...
public:
  poke_t(std::string node_, uint64_t value_): node(node_), value(value_) { }
  std::ostream& dump(std::ostream &os) const {
...
public:
  expect_t(std::string node_, uint64_t value_): node(node_), value(value_) { }
  std::ostream& dump(std::ostream &os) const {
...
public:
  read_t(uint64_t addr_, uint64_t tag_): addr(addr_), tag(tag_) { }
  std::ostream& dump(std::ostream &os) const {
...
public:
  ...
  void add_mem(uint64_t addr, uint64_t data) {
    mem[addr] = data;
  }
