...
f1_dtm_t::~f1_dtm_t() {}
f1_dtm_t::f1_dtm_t(const std::vector<std::string>& args) : htif_t(args)
{
  int ret_code;
...
uint32_t f1_dtm_t::do_command(f1_dtm_t::req r)
{
  req_buf = r;
  //write request
...
void f1_dtm_t::select_hart(int hartsel) {
...
void f1_dtm_t::halt(int hartsel)
{
...
void f1_dtm_t::resume(int hartsel)
{
...
uint64_t f1_dtm_t::save_reg(unsigned regno)
{
...
void f1_dtm_t::restore_reg(unsigned regno, uint64_t val)
{
  uint32_t data[xlen/(8*4)];
  data[0] = (uint32_t) val;
  if (xlen > 32) {
...
uint32_t f1_dtm_t::run_abstract_command(uint32_t command,
...
size_t f1_dtm_t::chunk_align()
{
  return xlen / 8;
}

void f1_dtm_t::read_chunk(uint64_t taddr, size_t len, void* dst)
{
...
void f1_dtm_t::write_chunk(uint64_t taddr, size_t len, const void* src)
{  
  uint32_t prog[ram_words];
  uint32_t data[data_words];
...
void f1_dtm_t::die(uint32_t cmderr)
{
  const char * codes[] = {
    "OK",
    "BUSY",
...
void f1_dtm_t::clear_chunk(uint64_t taddr, size_t len)
{
  uint32_t prog[ram_words];
  uint32_t data[data_words];
...
uint64_t f1_dtm_t::write_csr(unsigned which, uint64_t data)
{
  return modify_csr(which, data, WRITE);
}

uint64_t f1_dtm_t::set_csr(unsigned which, uint64_t data)
{
  return modify_csr(which, data, SET);
}

uint64_t f1_dtm_t::clear_csr(unsigned which, uint64_t data)
{
  return modify_csr(which, data, CLEAR);
}

uint64_t f1_dtm_t::read_csr(unsigned which)
{
  return set_csr(which, 0);
}

uint64_t f1_dtm_t::modify_csr(unsigned which, uint64_t data, uint32_t type)
{
  halt(current_hart);
...
size_t f1_dtm_t::chunk_max_size()
{
  // Arbitrary choice. 4k Page size seems reasonable.
  return 4096;
}

uint32_t f1_dtm_t::get_xlen()
{
...
void f1_host_thread_main(void* arg)
{
  ((f1_dtm_t*)arg)->producer_thread();
}
...
void f1_dtm_t::start_host_thread()
{
  req_wait = false;
  resp_wait = false;

  target = context_t::current();
  host.init(f1_host_thread_main, this);
  host.switch_to();
}
...
void f1_dtm_t::return_resp(resp resp_bits){
  resp_buf = resp_bits;
  host.switch_to();
}
